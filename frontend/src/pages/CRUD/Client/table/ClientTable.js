...
const ClientTable = () => {
  ...
  const count = useSelector((store) => store.client.list.count);
  const modalOpen = useSelector((store) => store.client.list.modalOpen);
  const rows = useSelector((store) => store.client.list.rows);
  const idToDelete = useSelector((store) => store.client.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="client" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Client')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/client/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Client'
                : 'http://localhost:8080/api-docs/#/Client'
            }
          >
