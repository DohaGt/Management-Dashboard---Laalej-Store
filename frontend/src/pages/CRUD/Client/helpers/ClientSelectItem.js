...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/client/autocomplete`, { params });
  return response.data;
}

const ClientSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.client.hasPermissionToCreate,
});

export default connect(select)(ClientSelectItem);
