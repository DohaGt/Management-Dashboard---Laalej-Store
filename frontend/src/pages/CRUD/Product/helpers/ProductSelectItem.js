...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/product/autocomplete`, { params });
...
const select = (state) => ({
  hasPermissionToCreate: state.product.hasPermissionToCreate,
});

export default connect(select)(ProductSelectItem);
