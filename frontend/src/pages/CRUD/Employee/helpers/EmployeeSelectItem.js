...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/employee/autocomplete`, { params });
  return response.data;
}

const EmployeeSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.employee.hasPermissionToCreate,
});

export default connect(select)(EmployeeSelectItem);
