...
import * as dataFormat from 'pages/CRUD/Employee/table/EmployeeDataFormatters';
...
const EmployeeTable = () => {
  ...
  const count = useSelector((store) => store.employee.list.count);
  const modalOpen = useSelector((store) => store.employee.list.modalOpen);
  const rows = useSelector((store) => store.employee.list.rows);
  const idToDelete = useSelector((store) => store.employee.list.idToDelete);
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Employee'
                : 'http://localhost:8080/api-docs/#/Employee'
            }
          >
            API documentation for employee
          </LinkMaterial>
        </div>
      </Widget>
