...
import * as dataFormat from 'pages/CRUD/Adminhistory/table/AdminhistoryDataFormatters';

import actions from 'actions/adminhistory/adminhistoryListActions';
...
const AdminhistoryTable = () => {
  ...
  const count = useSelector((store) => store.adminhistory.list.count);
  const modalOpen = useSelector((store) => store.adminhistory.list.modalOpen);
  const rows = useSelector((store) => store.adminhistory.list.rows);
  const idToDelete = useSelector((store) => store.adminhistory.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="adminhistory" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Adminhistory')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/adminhistory/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/adminhistory/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Adminhistory'
                : 'http://localhost:8080/api-docs/#/Adminhistory'
            }
          >
            API documentation for adminhistory
          </LinkMaterial>
        </div>
      </Widget>
