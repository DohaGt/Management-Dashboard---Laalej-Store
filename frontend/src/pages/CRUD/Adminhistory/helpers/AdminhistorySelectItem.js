...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/adminhistory/autocomplete`, { params });
  return response.data;
}

const AdminhistorySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.adminhistory.hasPermissionToCreate,
});

export default connect(select)(AdminhistorySelectItem);
