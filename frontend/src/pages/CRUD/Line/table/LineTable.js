...
const LineTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Line Number', title: 'lineNumber', number: 'true' },
  ...
  const count = useSelector((store) => store.line.list.count);
  const modalOpen = useSelector((store) => store.line.list.modalOpen);
  const rows = useSelector((store) => store.line.list.rows);
  const idToDelete = useSelector((store) => store.line.list.idToDelete);
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Line')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/line/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/line/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Line'
                : 'http://localhost:8080/api-docs/#/Line'
            }
          >
            API documentation for line
          </LinkMaterial>
        </div>
      </Widget>
