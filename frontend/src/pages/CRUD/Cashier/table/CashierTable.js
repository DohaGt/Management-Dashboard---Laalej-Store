...
const CashierTable = () => {
  ...
  const count = useSelector((store) => store.cashier.list.count);
  const modalOpen = useSelector((store) => store.cashier.list.modalOpen);
  const rows = useSelector((store) => store.cashier.list.rows);
  const idToDelete = useSelector((store) => store.cashier.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="cashier" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Cashier')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/cashier/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/cashier/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Cashier'
                : 'http://localhost:8080/api-docs/#/Cashier'
            }
          >
            API documentation for cashier
          </LinkMaterial>
        </div>
      </Widget>
