...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/salesperson/autocomplete`, { params });
  return response.data;
}

const SalespersonSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.salesperson.hasPermissionToCreate,
});

export default connect(select)(SalespersonSelectItem);
