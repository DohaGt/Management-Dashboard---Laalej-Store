...
import * as dataFormat from 'pages/CRUD/Salesperson/table/SalespersonDataFormatters';

// eslint-disable-next-line
import * as employeeDataFormat from 'pages/CRUD/Employee/table/EmployeeDataFormatters';

import actions from 'actions/salesperson/salespersonListActions';
...
const SalespersonTable = () => {
  ...
  const [filters, setFilters] = React.useState([
          ...
          {label: 'Commission', title: 'commission', number: 'true'},
  ...
  const count = useSelector((store) => store.salesperson.list.count);
  const modalOpen = useSelector((store) => store.salesperson.list.modalOpen);
  const rows = useSelector((store) => store.salesperson.list.rows);
  const idToDelete = useSelector((store) => store.salesperson.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="salesperson" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Salesperson')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/salesperson/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/salesperson/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Salesperson'
                : 'http://localhost:8080/api-docs/#/Salesperson'
            }
          >
            API documentation for salesperson
          </LinkMaterial>
        </div>
      </Widget>
