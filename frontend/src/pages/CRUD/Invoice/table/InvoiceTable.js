...
import * as dataFormat from 'pages/CRUD/Invoice/table/InvoiceDataFormatters';

import actions from 'actions/invoice/invoiceListActions';
...
const InvoiceTable = () => {
  ...
  const [filters, setFilters] = React.useState([
          ...
          {label: 'Invoice Number', title: 'invoiceNumber', number: 'true'},{label: 'Client Code', title: 'clientCode', number: 'true'},{label: 'Employee ID', title: 'employeeID', number: 'true'},
  ...
  const count = useSelector((store) => store.invoice.list.count);
  const modalOpen = useSelector((store) => store.invoice.list.modalOpen);
  const rows = useSelector((store) => store.invoice.list.rows);
  const idToDelete = useSelector((store) => store.invoice.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="invoice" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Invoice')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/invoice/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Invoice'
                : 'http://localhost:8080/api-docs/#/Invoice'
            }
          >
